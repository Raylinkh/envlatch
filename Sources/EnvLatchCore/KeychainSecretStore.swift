...
public enum KeychainStoreError: Error, LocalizedError, Sendable {
    ...
    public var errorDescription: String? {
        ...
        case .invalidStoredName(let name):
            return "Keychain contains an unsafe EnvLatch item named \(name). Remove it in Keychain Access."
        case .invalidStoredValue(let name):
            return "Keychain contains an invalid value for \(name). Replace it in EnvLatch."
        }
    }
}
    ...
    static func applicationOnly(descriptor: String) throws -> SecAccess {
        ...
        guard trustedStatus == errSecSuccess, let trustedApplication else {
            throw KeychainStoreError.osStatus(
                operation: "identify the EnvLatch executable",
        ...
        let accessStatus = SecAccessCreate(
            "EnvLatch · \(descriptor)" as CFString,
...
public struct KeychainSecretStore: SecretStore {
    public static let productionService = PersistenceNamespace.keychainService
