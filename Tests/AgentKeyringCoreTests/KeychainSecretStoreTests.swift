...
struct KeychainSecretStoreTests {
    @Test func listQueryRequestsAttributesButNeverSecretData() {
        let query = KeychainQueryFactory.list(service: "dev.agentkeyring.query-test")

        #expect(query[kSecClass as String] as! CFString == kSecClassGenericPassword)
        #expect(query[kSecAttrService as String] as? String == "dev.agentkeyring.query-test")
    ...
    @Test func itemQueryPinsExactAccountAndNonSyncScope() throws {
        let name = try CredentialName(validating: "AGENT_KEYRING_TEST_TOKEN")
        let query = KeychainQueryFactory.item(service: "dev.agentkeyring.query-test", name: name)
    ...
    @Test func addQueryCarriesAnExplicitApplicationOnlyACL() throws {
        ...
        let query = KeychainQueryFactory.add(
            service: "dev.agentkeyring.query-test",
            name: name,
    ...
    @Test func realKeychainRoundTripReplaceAndIdempotentDelete() throws {
        let service = "dev.agentkeyring.tests.\(UUID().uuidString)"
