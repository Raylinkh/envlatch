...
SDKs, scripts, tests, or backends with a named least-privilege environment:
...
skill. Add credentials only through the installed app after verifying the
download.

## Included

- one provider-agnostic `envlatch run --using` command;
- one- or multi-key launch profiles;
