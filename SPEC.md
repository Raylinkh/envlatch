...
- **AK-8 — Native bilingual GUI:** The app follows the macOS preferred language by default and provides a native Settings window where the user can choose System Default, English, or Simplified Chinese. Changes apply immediately to EnvLatch content; system-owned app menus continue to follow macOS. The Settings window also shows the exact version/build and whether the running copy is installed or a development build. The native GUI, dialogs, validation, recovery guidance, and accessibility labels are localized. User-supplied names, provider and contract names, environment variables, URLs, commands, the reusable agent setup prompt, and all CLI output remain unchanged. Localized resources are bundled inside the distributable app and verified from the packaged candidate; no localization dependency is introduced.
