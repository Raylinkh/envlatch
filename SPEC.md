...
EnvLatch gives one Mac user a single secure place to enter API keys and one launch command that makes a least-privilege selection available to any agent, host, or local process. Every saved key works directly by name; an optional key group selects several saved keys when one command needs them. Endpoint metadata still belongs to one saved key, keeping provider, wire-contract, base-URL, and target environment-name mapping separate from access selection without provider-specific commands.

The selected slice is complete when the user can save named environment variables in the GUI, optionally configure per-key endpoint metadata, run a saved key directly, optionally configure a key group containing exactly the keys a multi-key command needs, optionally register a display name for setup status, and prove `envlatch run --using <saved-key-or-group> -- <command>` supplies only that environment without EnvLatch printing or writing a secret. Broad `envlatch run --` remains an explicitly labeled compatibility utility and cannot satisfy release acceptance.
...
| 5 | Optional key group | When one command needs several keys, name that exact set | Membership names and derived endpoint bindings only | Edit, cancel, or remove missing/conflicting members |
| 6 | Optional setup registration | Agent or host supplies its own display name | Dynamic named-host setup list, explicitly non-authorizing | Repeat the idempotent pair command or skip it |
| 7 | Launch agent | Resolve and validate the selected key or group, then load only its exact keys and replace the wrapper process | No secret output | Nonzero exit before secret reads when metadata is invalid |
| 8 | Rotate or remove | Replace through secure field; removal is blocked while a key group references the key | Updated name list only | Remove membership first or cancel |
...
6. Rotation is stable because membership references the account name. Deletion is rejected while any key group references the account; the user removes membership first.
