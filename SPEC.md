...
- **AK-8 — Native bilingual GUI:** The app follows the macOS preferred language and provides complete English and Simplified Chinese copy for the native GUI, dialogs, validation, recovery guidance, and accessibility labels. User-supplied names, provider and contract names, environment variables, URLs, commands, the reusable agent setup prompt, and all CLI output remain unchanged. Localized resources are bundled inside the distributable app and verified from the packaged candidate; no custom language setting or localization dependency is introduced.
