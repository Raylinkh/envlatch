...
app_name="AgentKeyring.app"
dist_dir="$project_root/dist"
output_app="$dist_dir/$app_name"
signing_identity="${AGENT_KEYRING_CODESIGN_IDENTITY:--}"

cd "$project_root"
swift build -c release --product AgentKeyring
binary_dir=$(swift build -c release --show-bin-path)

stage_root=$(mktemp -d)
stage_app="$stage_root/$app_name"
mkdir -p "$stage_app/Contents/MacOS" "$stage_app/Contents/Resources"
install -m 755 "$binary_dir/AgentKeyring" "$stage_app/Contents/MacOS/AgentKeyring"
...
ditto "$project_root/AgentSkill/agent-keyring" "$stage_app/Contents/Resources/agent-keyring-skill"
...
if [[ -e "$output_app" ]]; then
  ...
  mv "$output_app" "$dist_dir/AgentKeyring.previous-$build_stamp.app"
fi
