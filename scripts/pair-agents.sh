...
if [[ -e "$legacy_cli_link" || -L "$legacy_cli_link" ]]; then
  mv "$legacy_cli_link" "$binary_dir/agent-keyring.previous-$pair_stamp"
fi
if [[ -e "$legacy_canonical_skill" || -L "$legacy_canonical_skill" ]]; then
  mv "$legacy_canonical_skill" "${legacy_canonical_skill:h}/agent-keyring.previous-$pair_stamp"
fi

if [[ -e "$cli_link" || -L "$cli_link" ]]; then
  current_cli="${cli_link:A}"
  if [[ "$current_cli" != "${agent_executable:A}" ]]; then
    mv "$cli_link" "$binary_dir/envlatch.previous-$pair_stamp"
  fi
fi
if [[ ! -e "$cli_link" && ! -L "$cli_link" ]]; then
...
if [[ -e "$canonical_skill" || -L "$canonical_skill" ]]; then
  if [[ ! -d "$canonical_skill" ]] || ! diff -qr "$skill_source" "$canonical_skill" >/dev/null; then
    mv "$canonical_skill" "${canonical_skill:h}/envlatch.previous-$pair_stamp"
  fi
fi
if [[ ! -e "$canonical_skill" && ! -L "$canonical_skill" ]]; then
...
for agent_home in "$user_home/.codex" "$user_home/.claude" "$user_home/.gemini"; do
  ...
  if [[ -e "$legacy_skill_link" || -L "$legacy_skill_link" ]]; then
    mv "$legacy_skill_link" "$skill_parent/agent-keyring.previous-$pair_stamp"
  fi
  if [[ -e "$skill_link" || -L "$skill_link" ]]; then
    current_skill="${skill_link:A}"
    if [[ "$current_skill" != "${canonical_skill:A}" ]]; then
      mv "$skill_link" "$skill_parent/envlatch.previous-$pair_stamp"
    fi
  fi
  if [[ ! -e "$skill_link" && ! -L "$skill_link" ]]; then
